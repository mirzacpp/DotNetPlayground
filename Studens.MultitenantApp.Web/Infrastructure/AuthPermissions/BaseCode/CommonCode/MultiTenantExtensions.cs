...
namespace Rev.AuthPermissions.BaseCode.CommonCode;
...
public static class MultiTenantExtensions
{
	...
	public static string GetTenantDataKey(this string tenantId, bool hasItsOwnDb)
	{
		if (tenantId == default)
			throw new AuthPermissionsException(
				"The Tenant DataKey is only correct if the tenant primary key is set");

		return hasItsOwnDb ? tenantId : DataKeyNoQueryFilter;
	}
	...
	{
		return tenant.TenantId.GetTenantDataKey(tenant.HasOwnDb);
	}
	...
	public static string GetHighestTenantId(this string tenantId)
	{
		if (tenantId == default)
			throw new AuthPermissionsException(
				"The Tenant DataKey is only correct if the tenant primary key is set");
