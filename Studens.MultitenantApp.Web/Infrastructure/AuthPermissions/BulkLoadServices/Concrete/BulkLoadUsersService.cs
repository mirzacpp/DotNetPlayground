...
namespace Rev.AuthPermissions.BulkLoadServices.Concrete
{
    ...
    public class BulkLoadUsersService : IBulkLoadUsersService
    {
        ...
        private async Task<IStatusGeneric> CreateUserTenantAndAddToDbAsync(BulkLoadUserWithRolesTenant userDefine, int index)
        {
            ...
            {
                var userInfo = await findUserInfoService.FindUserInfoAsync(userDefine.UserName);
            ...
            if (userId == null)
                return status.AddError(userDefine.UserName.FormErrorString(index - 1, -1,
            ...
            var authUserStatus = User.CreateUser(userId, userDefine.Email, userName, rolesToPermissions, userTenant);
