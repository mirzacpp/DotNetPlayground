...
namespace Rev.AuthPermissions.AdminCode
{
	...
	public interface IUsersAdminService
	{
		...
		IQueryable<User> QueryUsers(string dataKey = null);
		...
		Task<IStatusGeneric<User>> FindUserByUserIdAsync(string userId);
		...
		/// <returns>Status containing the User with UserRoles and UserTenant, or errors</returns>
		Task<IStatusGeneric<User>> FindUserByEmailAsync(string email);
