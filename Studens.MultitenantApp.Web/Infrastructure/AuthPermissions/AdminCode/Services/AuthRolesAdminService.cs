...
using Rev.AuthPermissions.AdminCode.Services.Internal;
...
namespace Rev.AuthPermissions.AdminCode.Services
{
	...
	public class AuthRolesAdminService : IAuthRolesAdminService
	{
				...
				: MapToRoleWithPermissionNamesDto(_context.RoleToPermissions
					.Where(x => x.RoleType == RoleTypes.Normal
								|| (x.RoleType == RoleTypes.TenantAutoAdd || x.RoleType == RoleTypes.TenantAdminAdd)
								   & x.Tenants.Select(y => y.TenantId).Contains(tenantId)));
		}
		...
		/// </summary>
		public IQueryable<User> QueryUsersUsingThisRole(string roleName)
		{
		...
		public async Task<IStatusGeneric> DeleteRoleAsync(string roleName, bool removeFromUsers)
		{
			...
			if (!removeFromUsers)
			{
				if (usersWithRoles.Any())
					status.AddError($"That role is used in {usersWithRoles.Count} Users and you didn't confirm the delete.",
