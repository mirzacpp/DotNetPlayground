...
namespace Rev.AuthPermissions.SupportCode.AddUsersServices.Authentication;
...
{
    private readonly IUsersAdminService _authUsersAdmin;
    ...
    public IndividualUserAddUserManager(IUsersAdminService authUsersAdmin,
    ...
    public async Task<IStatusGeneric> CheckNoExistingAuthUserAsync(AddNewUserDto newUser)
    {
        ...
        if ((await _authUsersAdmin.FindUserByEmailAsync(newUser.Email))?.Result != null)
    ...
    public async Task<IStatusGeneric> SetUserInfoAsync(AddNewUserDto newUser)
    {
        ...
        var tenantName = newUser.TenantId == null
            ? null
            : (await _tenantAdminService.GetTenantViaIdAsync(newUser.TenantId)).Result?.TenantName;
