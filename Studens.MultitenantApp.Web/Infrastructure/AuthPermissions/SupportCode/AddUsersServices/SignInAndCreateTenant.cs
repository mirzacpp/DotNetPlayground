...
using Rev.AuthPermissions.SupportCode.AddUsersServices.Authentication;
using Rev.AuthPermissions.SupportCode.ShardingServices;
using Microsoft.EntityFrameworkCore;
using StatusGeneric;

namespace Rev.AuthPermissions.SupportCode.AddUsersServices;
...
public class SignInAndCreateTenant : ISignInAndCreateTenant
{
    ...
    public SignInAndCreateTenant(
		AuthPermissionsOptions options,
		IAuthTenantAdminService tenantAdmin,
        ...
        AddNewTenantDto tenantData, MultiTenantVersionData versionData)
    {
        ...
        if (await _tenantAdmin.QueryTenants().AnyAsync(x => x.TenantName == tenantData.TenantName))
		...
		var tenantStatus = await _tenantAdmin.AddSingleTenantAsync(tenantData.TenantName, tenantRoles, hasOwnDb, databaseInfoName);
        ...
        status.Message =
            $"Successfully created the tenant '{tenantStatus.Result.TenantName}' and registered you as the tenant admin.";
