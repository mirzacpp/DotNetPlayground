...
using StatusGeneric;
using Rev.AuthPermissions.AspNetCore.AccessTenantData;
...
public class LinkToTenantDataService : ILinkToTenantDataService
{
    ...
    public async Task<IStatusGeneric> StartLinkingToTenantDataAsync(string currentUserId, string tenantId)
    {
        ...
        status.Message = $"You are now linked the the data of the tenant called '{tenantToLinkTo.TenantName}'";
        return status;
    }
    ...
    private string EncodeCookieContent(Tenant tenantToLinkToTenant)
    {
        var values = _options.TenantType.IsSharding()
            ? $"{tenantToLinkToTenant.GetTenantDataKey()},{tenantToLinkToTenant.DatabaseInfoName},{tenantToLinkToTenant.TenantName}"
            : $"{tenantToLinkToTenant.GetTenantDataKey()},{tenantToLinkToTenant.TenantName}";
